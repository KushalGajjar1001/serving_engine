...
def main():
    ...
    # ---------------------------------------------------------
    #   PRINT CLEAN TABLE + SAVE CSV
    # ---------------------------------------------------------

    df = pd.DataFrame(results)

    # Derived metrics
    df["throughput_opt"] = df["optimized_tokens"] / df["optimized_e2e"]
    df["throughput_naive"] = df["naive_tokens"] / df["naive_e2e"]
    df["ttft_opt"] = df["optimized_e2e"] / df["max_tokens"]
    df["ttft_naive"] = df["naive_e2e"] / df["max_tokens"]
